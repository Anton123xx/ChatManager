...
namespace ChatManager.Controllers
{
    public class AccountsController : Controller
    {
        ...
        //        }
        //        return (int)Session["UserToEdit"];
        //    }
        //    set
        //    {
        //        Session["UserToEdit"] = value;
        //    }
        //}
        ...
        [OnlineUsers.UserAccess]
        public ActionResult AdminModifierProfil(int id)
        {
            //OnlineUsers.GetSessionUser().AcceptNotification = true;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken()]
        public ActionResult AdminModifierProfil(User userToEdit)
        {

            //prenom. nom, courriel, role, statut de verif, bloqué, avatar
            User currentUser = DB.Users.FindUser(userToEdit.Id);
            userToEdit.Id = currentUser.Id;
            userToEdit.Verified = currentUser.Verified;
            userToEdit.UserTypeId = currentUser.UserTypeId;
            userToEdit.Blocked = currentUser.Blocked;
            userToEdit.Avatar = currentUser.Avatar;
            userToEdit.CreationDate = currentUser.CreationDate;

            string newEmail = "";
            //string newFirstName = "";
            //string newLastName = "";
            //int newRole = 999;
            //bool newAccountState = 999;
            if (!ModelState.IsValid)
            {
                ...
                {
                   // DB.Users.FindUser(userToEdit.Id).Blocked = true;
                   
                }
                    


                if (userToEdit.Email != currentUser.Email)
                {
                    newEmail = userToEdit.Email;
                    userToEdit.Email = userToEdit.ConfirmEmail = currentUser.Email;
                }


              

                if (DB.Users.Update(userToEdit))
                {
                    if (newEmail != "")
                    {
                        SendEmailChangedVerification(userToEdit, newEmail);
                        return RedirectToAction("EmailChangedAlert");
                    }
                    else
                    ...
                    return RedirectToAction("Report", "Errors", new { message = "Échec de modification de profil par l'Admin" });
            }
            ViewBag.UserType = SelectListUtilities<UserType>.Convert(DB.UserTypes.ToList());
